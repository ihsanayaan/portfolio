  ...
  <SectionHeading title="🛠 Projects" />
    ...
    <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
      {projects.map((project, index) => (
        <div
          key={index}
          className="bg-bgDark p-4 rounded-soft border border-borderLight shadow-md transition hover:shadow-lg"
        >
          <img
            src={project.image}
            alt={project.title}
            className="w-full h-40 object-cover rounded-md mb-4 border border-textSub"
          />
          <h3 className="text-xl font-semibold text-accentLight mb-2">{project.title}</h3>
          <p className="text-sm text-textSub mb-4">{project.description}</p>
